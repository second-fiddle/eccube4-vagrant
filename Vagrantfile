...
GUEST_IP_ADDR = "192.168.2.100"
PATH_MAPPINGS = {
  eccube:   { host: "./ec-cube" ,  guest: "/vagrant/ec-cube" },
}
...
Vagrant.configure("2") do |config|
  ...
  config.vm.synced_folder PATH_MAPPINGS[:eccube][:host], PATH_MAPPINGS[:eccube][:guest],
  ...
  config.vm.provision :docker_compose,
   yml: "#{PATH_MAPPINGS[:eccube][:guest]}/docker-compose.yml",
   rebuild: true,
   run: "always"
end
